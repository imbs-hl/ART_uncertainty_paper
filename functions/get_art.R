# Function to build an artificial representative tree (ART) with batchtools (as algorithm input)
# for continous outcome glycohemoglobin based on the previously trained RF (instance input)
# conformal predictive systems are used for uncertainty quantification
# and to calculate a cummukative distribution function to predict the probaility for diabetes and prediabetes
# in each leaf
...
get_art <- function(data, instance, metric, probs_quantiles, epsilon, min.bucket = 0, significance_level = 0.1, ...){
  ...
  #----------------
  return(list(info_df = data.frame(method     = "Regression ART + CPS",
                         metric               = metric, 
