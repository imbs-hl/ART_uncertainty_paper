get_seperate_art <- function(data, instance, metric, probs_quantiles, epsilon, min.bucket = 0, significance_level = 0.1, ...){
  source("/imbs/home/kronziel/ART_uncertainty/functions/new_distance_measures.R")
  ...
  rf_prob5.7 <- instance[[6]]
  rf_prob6.5 <- instance[[7]]
  ...
  #----------------
  start <- proc.time()
  # Regression ART trainieren
  art_regr <- generate_tree(rf = rf, metric = metric, train_data = train_data, test_data = train_data, 
  ...
  # Probability ART for prädiabetes trainieren
  train_data_5.7 <- train_data %>% 
    mutate(glycohemoglobin = ifelse(glycohemoglobin>=5.7,1,0))
  
  art_prob5.7 <- generate_tree(rf = rf_prob5.7, metric = metric, train_data = train_data_5.7, test_data = train_data_5.7, 
                               dependent_varname = "glycohemoglobin", 
                               probs_quantiles = probs_quantiles, min.bucket=min.bucket)

  # Probability ART for Diabetes trainieren
  train_data_6.5 <- train_data %>% 
    mutate(glycohemoglobin = ifelse(glycohemoglobin>=6.5,1,0))
  
  art_prob6.5 <- generate_tree(rf = rf_prob6.5, metric = metric, train_data = train_data_6.5, test_data = train_data_6.5, 
                               dependent_varname = "glycohemoglobin", 
                               probs_quantiles = probs_quantiles, min.bucket=min.bucket)
  ...
  #----------------
  return(list(info_df = data.frame(method     = "Regression ART + Probability ARTs",
                         metric               = metric, 
