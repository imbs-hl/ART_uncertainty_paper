calculate_mrt_rep_tree <- function(data, instance, metric, significance_level = 0.1, calibrate_all_nodes = FALSE, ...){
  ...
  start <- proc.time()
  ## Generate most rep tree
  dist_matrix <- measure_distances(rf = rf, metric = metric, test_data = metric_test_data)
  rf_rep <- select_trees(rf = rf, distance.matrix = dist_matrix, num.trees = 1)
  end <- proc.time()
  ...
  return(data.frame(metric               = metric, 
                    method               = "most representative tree with uncertainty",
